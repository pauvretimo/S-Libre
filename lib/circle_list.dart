...
typedef snapIndex = int? Function(int index);
...
class _CircleListState extends State<CircleList>
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(
      ...
      if (widget.snapping != null) {
        snapper.snappingNb = widget.snapping;
        snapper.snap = true;
        List<double> snapList = snapper.list();
        snapper.getPoint(dragModel.angleDiff);
      }
      return SizedBox(
        width: outerRadius * 2,
            ...
            Positioned(
              ...
              child: RadialDragGestureDetector(
                ...
                onRadialDragUpdate: (PolarCoord updateCoord) {
                  ...
                  if (widget.onSnapUpdate != null) {
                    widget.onSnapUpdate!(snapper.point);
                  }
                },
              ...
              child: SizedBox(
                ...
                child: RadialDragGestureDetector(
                  ...
                  child: Transform.rotate(
                    angle: snapper.snap
                        ? (isAnimationStop
                            ? (snapper.snapList[snapper.point] +
                                widget.initialAngle)
...
class _Snapping {
  int? snappingNb = 1;
  bool snap = false;
  late List<double> snapList;
  int point = 0;

  List<double> list() {
    snapList = List.generate(snappingNb!, (index) {
      return (2 * pi * index / snappingNb!);
    });
    return snapList;
  }

  int getPoint(double angle) {
    point = ((snappingNb! * angle / 2 / pi + 0.5).truncate() % snappingNb!);
    return point;
  }
}
